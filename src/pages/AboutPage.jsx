    ...
    <Box sx={{ minHeight: '100vh', background: 'linear-gradient(160deg,#070D18 0%,#0D1B2A 60%,#070D18 100%)', position: 'relative', overflow: 'hidden' }}>
      ...
      <Container maxWidth="lg" sx={{ mb: 10 }}>
        ...
        <Box sx={{
          ...
          {team.map(({ name, role, photo, initials, color, gradient, bio }) => (
            <Card key={name} elevation={0} sx={{
              textAlign: 'center',
      ...
      <Box sx={{ py: 4, textAlign: 'center' }}>
        ...
        <Typography variant="caption" sx={{ color: '#334155' }}>
          © 2025 FinGuard. Built with ❤️ by the FinGuard Team · LNBTI SEM4
        </Typography>
      </Box>
    </Box>
  );
